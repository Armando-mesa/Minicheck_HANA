...
- ABAP parameter checks (A05xx) are based on table PAHI that is only updated from time to time, so after parameter changes 
  you may have to wait for a day until the check recognizes the new parameter value (SAP Note 2960117).
...
( SELECT -1 CHECK_ID, '' NAME,                  '' DESCRIPTION,                                    '' SAP_NOTE, '' EXPECTED_OP, '' EXPECTED_VALUE FROM DUMMY WHERE 1 = 0 UNION ALL
  ...
  ( SELECT   13, 'ABAP_KERNEL_VERSION',          'ABAP kernel version',                             '19466',   'any',      'any'            FROM DUMMY ) UNION ALL
  ...
  ( SELECT  265, 'MANUAL_CHECK',                 'Catalog buffer swaps per day (mid-term)',         '1918603', '<=',       '1000'           FROM DUMMY ) UNION ALL
  ...
  ( SELECT  702, 'MANUAL_CHECK',                 'Current requests in dispatcher queue',            '',        '<=',       '30'             FROM DUMMY ) UNION ALL
  ( SELECT  703, 'MANUAL_CHECK',                 'Maximum requests in dispatcher queue',            '',        '<=',       '500'            FROM DUMMY ) UNION ALL
  ( SELECT  704, 'MANUAL_CHECK',                 'Existence of kernel snapshots (mid-term)',        '',        '=',        '0'              FROM DUMMY ) UNION ALL
  ( SELECT  705, 'MANUAL_CHECK',                 'Work processes waiting for ABAP semaphore',       '',        '<=',       '10'             FROM DUMMY ) UNION ALL
  ( SELECT  710, 'MANUAL_CHECK',                 'Work processes busy requesting enqueue',          '2013043', '<=',       '5'              FROM DUMMY ) UNION ALL
  ( SELECT  720, 'MANUAL_CHECK',                 'Additionally activated work process traces',      '',        '=',        'none'           FROM DUMMY ) UNION ALL
  ...
  ( SELECT  865, 'LARGE_JOB_LOGS',               'Batch jobs with large job logs',                  '',        '=',        'none'           FROM DUMMY ) UNION ALL

  ( SELECT  900, 'SQLM_ACTIVE',                  'SQLM activated',                                  '1885926', '=',        'yes'            FROM DUMMY ) UNION ALL
  ( SELECT  905, 'MANUAL_CHECK',                 '/SDF/SMON activated',                             '3007524', '=',        'yes'            FROM DUMMY ) UNION ALL

  ( SELECT 1000, 'DATA_TARGETS_MANY_REQUESTS',   'BW data targets with many requests',              '1431315', '=',        '0'              FROM DUMMY ) UNION ALL
  ( SELECT 1001, 'RSPMREQUEST_MANY_REQUESTS',    'BW data targets with many RSPMREQUEST entries',   '2388483', '=',        'none'           FROM DUMMY ) UNION ALL
  ( SELECT 1005, 'HANA_OPTIMIZED_DSOS',          'Number of HANA-optimized DSOs',                   '1849497', '=',        '0'              FROM DUMMY ) UNION ALL
  ( SELECT 1010, 'MD_LOOKUP_PREFETCH_LOGIC',     'BW master data lookup prefetch active',           '1092539', '=',        'yes'            FROM DUMMY ) UNION ALL
  ...
  ( SELECT 1220, 'LARGE_CUSTOMER_TABLES',        'Large ABAP customer tables',                      '',        '=',        'none'           FROM DUMMY ) UNION ALL
  ...
  ( SELECT 1700, 'CLIENT_CHANGES_ALLOWED',       'Production clients with changes allowed',         '',        '=',        'none'           FROM DUMMY ) UNION ALL
  ( SELECT 1701, 'REPOSITORY_CHANGES_ALLOWED',   'Prod. clients with cross-client changes allowed', '',        '=',        'none'           FROM DUMMY ) UNION ALL
  ( SELECT 1702, 'MODIFIABLE_CHANGE_OPTION',     'System change option set to modifiable',          '',        '=',        'no'             FROM DUMMY ) UNION ALL
...
SELECT
  CASE 
    ...
    WHEN ONLY_POTENTIALLY_CRITICAL_RESULTS = 'X' OR ROW_NUM = 1 THEN LPAD(MAP(CHECK_ID_PREFIX, '', TO_VARCHAR(CHECK_ID), CHECK_ID_PREFIX || LPAD(CHECK_ID, 4, 0)), 5) 
    ELSE '' 
  END CHID,
...
( SELECT
  ...
  ( ( SELECT
      ...
      FROM
        ...
        ( SELECT 
          ...
          WHERE
            ...
            TABLE_NAME NOT IN ('SVER', 'SVED', 'SVET', 'SVEP', 'CVEP1', 'CVEP11', 'CVEP12', 'CVER4', 'CVER11', 'CVER12', 'TACOPA', 'TACOPAB', 'TACOPB', 'TACOPBA' ) AND        /* SAP Note 2839111 */
            RECORD_COUNT >= 1
        ) ON
          1 = 1
    )
    ...
    ( SELECT
        CHECK_NAME,
        CASE
          WHEN PARAMETER_NAME = 'rsdb/prefer_join_with_fda'    AND ( VERSION = 1 AND REVISION <= 122.20 OR VERSION = 2 AND REVISION <= 19.99 ) AND VALUE = '0'  THEN 'okay' || CHAR(32) || '(' || VALUE || ')'
          WHEN PARAMETER_NAME = 'rsdb/prefer_join_with_fda'    AND ( VERSION = 1 AND REVISION <= 122.20 OR VERSION = 2 AND REVISION <= 19.99 ) AND VALUE != '0' THEN 'wrong' || CHAR(32) || '(' || VALUE || CHAR(32) || 'instead of' || CHAR(32) || '0)'
          WHEN PARAMETER_NAME = 'rsdb/prefer_join_with_fda'    AND ( VERSION = 2 AND REVISION BETWEEN 20 AND 39 OR VERSION = 1 AND REVISION >= 122.21 ) THEN 'okay' || CHAR(32) || '(' || VALUE || ')'
          WHEN PARAMETER_NAME = 'rsdb/prefer_join_with_fda'    AND ( VERSION >= 3 OR VERSION = 2 AND REVISION >= 40 ) AND VALUE IN ( '1', 'not set' ) THEN 'okay' || CHAR(32) || '(' || VALUE || ')'
          WHEN PARAMETER_NAME = 'rsdb/prefer_join_with_fda'    AND ( VERSION >= 3 OR VERSION = 2 AND REVISION >= 40 ) AND VALUE NOT IN ( '1', 'not set' ) THEN 'wrong' || CHAR(32) || '(' || VALUE || CHAR(32) || 'instead of' || CHAR(32) || '1)'
          WHEN PARAMETER_NAME = 'dbs/hdb/prefer_join_with_fda' AND ( VERSION = 1 AND REVISION <= 122.20 OR VERSION = 2 AND REVISION <= 19.99 ) AND VALUE = '0' THEN 'okay' || CHAR(32) || '(' || VALUE || ')'
          WHEN PARAMETER_NAME = 'dbs/hdb/prefer_join_with_fda' AND ( VERSION = 1 AND REVISION <= 122.20 OR VERSION = 2 AND REVISION <= 19.99 ) AND VALUE != '0' THEN 'wrong' || CHAR(32) || '(' || VALUE || CHAR(32) || 'instead of' || CHAR(32) || '0)'
          WHEN PARAMETER_NAME = 'dbs/hdb/prefer_join_with_fda' AND ( VERSION = 2 AND REVISION BETWEEN 20 AND 39 OR VERSION = 1 AND REVISION >= 122.21 ) THEN 'okay' || CHAR(32) || '(' || VALUE || ')'
          WHEN PARAMETER_NAME = 'dbs/hdb/prefer_join_with_fda' AND ( VERSION >= 3 OR VERSION = 2 AND REVISION >= 40 ) AND VALUE IN ( '1', 'not set' ) THEN 'okay' || CHAR(32) || '(' || VALUE || ')'
          WHEN PARAMETER_NAME = 'dbs/hdb/prefer_join_with_fda' AND ( VERSION >= 3 OR VERSION = 2 AND REVISION >= 40 ) AND VALUE NOT IN ( '1', 'not set' ) THEN 'wrong' || CHAR(32) || '(' || VALUE || CHAR(32) || 'instead of' || CHAR(32) || '1)'
          WHEN PARAMETER_NAME = 'rsdb/supports_fda_prot'       AND ( VERSION = 1 AND REVISION = 122.14 OR VERSION = 2 AND REVISION = 12.03 ) AND VALUE = '0' THEN 'okay' || CHAR(32) || '(' || VALUE || ')'
          WHEN PARAMETER_NAME = 'rsdb/supports_fda_prot'       AND ( VERSION = 1 AND REVISION = 122.14 OR VERSION = 2 AND REVISION = 12.03 ) AND VALUE != '0' THEN 'wrong' || CHAR(32) || '(' || VALUE || CHAR(32) || 'instead of' || CHAR(32) || '0)'
          WHEN PARAMETER_NAME = 'rsdb/supports_fda_prot'       AND NOT ( VERSION = 1 AND REVISION = 122.14 OR VERSION = 2 AND REVISION = 12.03 ) AND VALUE = '1' THEN 'okay' || CHAR(32) || '(' || VALUE || ')'
          WHEN PARAMETER_NAME = 'rsdb/supports_fda_prot'       AND NOT ( VERSION = 1 AND REVISION = 122.14 OR VERSION = 2 AND REVISION = 12.03 ) AND VALUE != '1' THEN 'wrong' || CHAR(32) || '(' || VALUE || CHAR(32) || 'instead of' || CHAR(32) || '1)'
          WHEN PARAMETER_NAME = 'dbs/hdb/supports_fda_prot'    AND ( VERSION = 1 AND REVISION = 122.14 OR VERSION = 2 AND REVISION = 12.03 ) AND VALUE = '0' THEN 'okay' || CHAR(32) || '(' || VALUE || ')'
          WHEN PARAMETER_NAME = 'dbs/hdb/supports_fda_prot'    AND ( VERSION = 1 AND REVISION = 122.14 OR VERSION = 2 AND REVISION = 12.03 ) AND VALUE != '0' THEN 'wrong' || CHAR(32) || '(' || VALUE || CHAR(32) || 'instead of' || CHAR(32) || '0)'
          WHEN PARAMETER_NAME = 'dbs/hdb/supports_fda_prot'    AND NOT ( VERSION = 1 AND REVISION = 122.14 OR VERSION = 2 AND REVISION = 12.03 ) AND VALUE = 'not set' THEN 'okay' || CHAR(32) || '(' || VALUE || ')'
          WHEN PARAMETER_NAME = 'dbs/hdb/supports_fda_prot'    AND NOT ( VERSION = 1 AND REVISION = 122.14 OR VERSION = 2 AND REVISION = 12.03 ) AND VALUE != 'not set' THEN 'wrong' || CHAR(32) || '(' || VALUE || CHAR(32) || 'instead of' || CHAR(32) || 'not set)'
    ...
    ( SELECT
        'SQLM_ACTIVE',
        CASE WHEN SECONDS_BETWEEN(TO_TIMESTAMP(SUBSTR(MAX(TIMESTAMP_START), 1, 14), 'YYYYMMDDHH24MISS'), CURRENT_TIMESTAMP) <= 43200 THEN 'yes' ELSE 'no' END
      FROM
        SQLM_ACTIVITIES
    )
    UNION ALL
    ...
    ( SELECT TOP 5
        ...
        IFNULL(DATATARGET || CHAR(32) || '(' || NUM_REQUESTS || CHAR(32) || 'requests)', 'none')
      FROM
        DUMMY LEFT OUTER JOIN
      ( SELECT
          DATATARGET,
    ...
    ( SELECT
      ...
      WHERE
        ...
        ( '/SAPAPO/TSTROBJR', '/VIRSA/ZFFCDHDR', '/VIRSA/ZFFTNSLOG', '/VIRSA/ZVIRFFLOG', 'ALPFASSIGN', 'BKK40', 'BKK42', 'BKK45', 'BKK46', 'BKK98', 'BKKSONT',
          ...
          'TMY06', 'TRACT_POSCONTEXT', 'TRLT_DERIVFLOWS', 'TTSTR', 'T006', 'T006A', 'T006B', 'T006C', 'T006D', 'T006T', 'T006I', 'T006J', 'T009', 'T009B', 'T009C', 'T009Y', 'T009T', 
    ...
    ( SELECT
        ...
        TO_VARCHAR(TO_DECIMAL(SUM(TO_DECIMAL(LENGTH(XDATA))) / 1024 / 1024 / 1024, 10, 2))
      FROM
        RSWR_DATA R
      WHERE
        NOT EXISTS
        ( SELECT
          ...
          WHERE
            U.BNAME = R.OWNER AND
            ( U.GLTGB = '00000000' OR U.GLTGB >= TO_VARCHAR(CURRENT_TIMESTAMP, 'YYYYMMDD') )
        )
    )
    UNION ALL
    ( SELECT
    ...
    ( SELECT TOP 10
        'LARGE_JOB_LOGS',
        IFNULL(JOBNAME || CHAR(32) || '(' || TO_DECIMAL(DSIZE / 1024 / 1024, 10, 0) || 'M,' || CHAR(32) ||
           SUBSTR(SDLDATE, 1, 4) || '/' || SUBSTR(SDLDATE, 5, 2) || '/' || SUBSTR(SDLDATE, 7, 2) || CHAR(32) ||
           SUBSTR(SDLTIME, 1, 2) || ':' || SUBSTR(SDLTIME, 3, 2) || ':' || SUBSTR(SDLDATE, 5, 2) || ')', 'none')
      FROM
      ( SELECT
          TB.JOBNAME,
          TS.DSIZE,
    ...
    ( SELECT
        ...
        IFNULL(TABLE_NAME || CHAR(32) || '(' || TO_DECIMAL(DISK_SIZE / 1024 / 1024 / 1024, 10, 2) || CHAR(32) || 'GB)', 'none')
      FROM
        DUMMY LEFT OUTER JOIN
...
WHERE
  ONLY_POTENTIALLY_CRITICAL_RESULTS = ' ' OR MAP(NAME, 'MANUAL_CHECK', '', MAP(VALUE, 'n/a', '', POTENTIALLY_CRITICAL)) = 'X' OR M.CHECK_ID <= 10 OR M.CHECK_ID >= 9000
ORDER BY
  M.CHECK_ID,
  M.VALUE
