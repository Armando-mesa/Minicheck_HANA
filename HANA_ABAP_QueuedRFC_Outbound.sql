...
- Table TRFCQOUT only available in SAP ABAP environments
- You have to be connected to the SAP<sid> schema otherwise the following error is issued:

  [259]: invalid table name: Could not find table/view TRFCQOUT in schema
...
- 2024/04/30:  1.0 (initial version)
...
- QNAME
  ...
  'R3AD_CUSTOME'  --> qRFC queue name R3AD_CUSTOME
  '%'             --> No restriction related to qRFC queue name
...
( SELECT
    ...
    CASE WHEN BI.AGGREGATE_BY = 'NONE' OR INSTR(BI.AGGREGATE_BY, 'QNAME')    != 0 THEN Q.QNAME     ELSE MAP(BI.QNAME,    '%', 'any', BI.QNAME)    END QNAME,
  ...
  ( SELECT
    ...
    ( SELECT                  /* Modification section */
        ...
        '%' QRFCUSER,
        '%' ERRMESS,
        'QRFCFNAM, QRFCUSER, DEST, QNAME, ERRMESS, QSTATE' AGGREGATE_BY,           /* TIME, MAND, DEST, QNAME, QSTATE, QRFCUSER, QRFCFNAM, ERRMESS or comma separated combinations, NONE for no aggregation */
  ...
  GROUP BY
    ...
    CASE WHEN BI.AGGREGATE_BY = 'NONE' OR INSTR(BI.AGGREGATE_BY, 'QNAME')    != 0 THEN Q.QNAME     ELSE MAP(BI.QNAME,    '%', 'any', BI.QNAME)    END,
