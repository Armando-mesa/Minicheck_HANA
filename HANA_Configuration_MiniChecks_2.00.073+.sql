...
- HANA_Configuration_MiniChecks_2.00.073+
...
- In case long runtimes are observed with call stacks related to M_CONTEXT_MEMORY (TypedStatisticsWrapper__M_CONTEXT_MEMORY::traverseNodesImpl),
  you can check if adding the OPTIMIZATION_LEVEL(RULE_BASED) hint to the last line of the Mini Check improves the situation:
...
- 2023/08/26:  2.8 (dedicated 2.00.070+ version including consideration of retained data backups)
- 2023/12/01:  2.9 (SKIP_OVERLAPPING_CHECKS_IN_SYSTEMDB included)
- 2023/12/09:  3.0 (dedicated 2.00.073+ version including HOST_CS_LOB_SPACE_RECLAIM_DETAILS)
  ...
  ( SELECT
    ...
    ( SELECT
      ...
      ( SELECT
          ...
          ( SELECT SUM(COMPRESSED_SIZE) / 1024 / 1024 / 1024 FROM TEMP_M_BACKUP_CATALOG_FILES BF WHERE BF.BACKUP_ID = B.BACKUP_ID ) BACKUP_SIZE_GB
        FROM
...
*/
    ...
    ( SELECT
      ...
      ( SELECT TOP 5
        ...
        WHERE
          SECONDS_BETWEEN(T.TIMESTAMP, CURRENT_TIMESTAMP) <= 86400 * BI.SHORTTERM_DAYS AND
          T.STATEMENT_HASH != CHAR(63) AND
          T.THREAD_DETAIL NOT LIKE 'Waits for client to fetch%'
        GROUP BY
          T.STATEMENT_HASH,
          G.THREAD_SAMPLES_FILTER_FACTOR,
          BI.SHORTTERM_DAYS
    ...
    ( SELECT
      ...
      ( SELECT
        ...
        HAVING
          ...
          MAP(SUM(LOB_COUNT), 0, 0, SUM(BINARY_SIZE) / SUM(LOB_COUNT) / 1024) < 16
      ) ON
        1 = 1
    )
    UNION ALL
...
/* Available as of SAP HANA 2.00.030 */
    ...
    ( SELECT
      ...
      WHERE
        R.START_TIME >= ADD_SECONDS(CURRENT_TIMESTAMP, -86400 * BI.MIDTERM_DAYS)
    )
    UNION ALL
    ( SELECT
        'MANY_LOBS_RECLAIMED',
        '',
        IFNULL(TABLE_NAME || CHAR(32) || '(' || TO_DECIMAL(LOBS / 1000000 / BI.MIDTERM_DAYS, 10, 2) || CHAR(32) || 'Mio.)', 'none')
      FROM
        BASIS_INFO BI LEFT OUTER JOIN
      ( SELECT
          SCHEMA_NAME,
        ...
        ( SELECT DISTINCT
          ...
          FROM
            BASIS_INFO BI,
            _SYS_STATISTICS.HOST_CS_LOB_SPACE_RECLAIM_DETAILS R
          WHERE
            R.START_TIME >= ADD_SECONDS(CURRENT_TIMESTAMP, -86400 * BI.MIDTERM_DAYS)
        )
        GROUP BY
    ...
    UNION ALL
    ( SELECT
        'SUPPRESSED_JOBWORKERS',
        '',
        TO_VARCHAR(SUM(SUPPRESSED_WAITING_JOB_WORKER_COUNT))
      FROM
    ...
    UNION ALL
    ( SELECT
        'NSE_CACHE_RELOAD_PCT',
        C.HOST,
        TO_VARCHAR(TO_DECIMAL(SUM(MAP(MAX_SIZE, 0, 0, IO_READ_SIZE) / BI.MIDTERM_DAYS / MAX_SIZE * 100), 10, 0))
        FROM
        BASIS_INFO BI LEFT OUTER JOIN
        ( SELECT
            SERVER_TIMESTAMP,
            ...
            CACHE_NAME,
            MAX_SIZE,
            GREATEST(0, IO_READ_SIZE - LAG(IO_READ_SIZE) OVER (PARTITION BY HOST, PORT, CACHE_NAME ORDER BY SERVER_TIMESTAMP)) IO_READ_SIZE
          FROM
            _SYS_STATISTICS.HOST_BUFFER_CACHE_STATISTICS
        ) C ON
          SECONDS_BETWEEN(C.SERVER_TIMESTAMP, CURRENT_TIMESTAMP) <= 86400 * BI.MIDTERM_DAYS
      GROUP BY
        C.HOST,
        BI.MIDTERM_DAYS
    )
...
/* TMC_GENERATION_START_2 */
    ...
    ( SELECT   12, 'REVISION_LEVEL',               'Revision level',                                  '2378962', '>=',       '74.00',         -1,    -1 FROM DUMMY ) UNION ALL
    ...
    ( SELECT  375, 'MANY_LOBS_RECLAIMED',          'High table CS LOB reclaims (mid-term, per day)',  '2169283', '=',        'none',          -1,    -1 FROM DUMMY ) UNION ALL
    ...
    ( SELECT  855, 'CS_LOB_GC_ERRORS',             'CS LOB garbage collection errors (mid-term)',     '2169283', '=',        '0',             -1,    -1 FROM DUMMY ) UNION ALL
    ...
    ( SELECT  876, 'SUPPRESSED_JOBWORKERS',        'Suppressed JobWorker threads',                    '2222250', '<=',       '20',            -1,    -1 FROM DUMMY ) UNION ALL
    ...
    ( SELECT 2735, 'NSE_CACHE_RELOAD_PCT',         'NSE buffer cache reloads (% per day, mid-term)',  '2799997', '<=',       '100',           -1,    -1 FROM DUMMY ) UNION ALL
