...
- HANA_Global_DiffReport_2.00.070+
...
- 2023/10/29:  2.1 (dedicated 2.00.070+ version including APPLICATION_COMPONENT_NAME and APPLICATION_COMPONENT_TYPE)
...
- TOP_N_THREAD_APP_COMP_NAMES

  Number of top thread application component names to be displayed, top N is considered for both time intervals

  5              --> Display top 5 application component names for both time intervals
...
( SELECT 
  ...
  UNION ALL
  SELECT
    110700 + ROW_NUM / 10000, APPLICATION_SOURCE, THREADS_1, THREADS_2
  FROM
  ( SELECT
      ROW_NUMBER() OVER (ORDER BY T2.ELAPSED_H DESC, T1.ELAPSED_H DESC) ROW_NUM,
  ...
  UNION ALL
  SELECT 
    110800, 'TOTAL', THREADS_1, THREADS_2
  FROM
  ( SELECT SUM(ELAPSED_H) * 3600 / MAX(INTERVAL_S_1) THREADS_1 FROM BASIS_INFO, THREADS_1 ) T1,
  ( SELECT SUM(ELAPSED_H) * 3600 / MAX(INTERVAL_S_2) THREADS_2 FROM BASIS_INFO, THREADS_2 ) T2
  UNION ALL
  SELECT
    110900 + ROW_NUM / 10000, APPLICATION_COMPONENT_NAME, THREADS_1, THREADS_2
  FROM
  ( SELECT
      ROW_NUMBER() OVER (ORDER BY T2.ELAPSED_H DESC, T1.ELAPSED_H DESC) ROW_NUM,
      IFNULL(T2.APPLICATION_COMPONENT_NAME, T1.APPLICATION_COMPONENT_NAME) APPLICATION_COMPONENT_NAME,
      IFNULL(T1.ELAPSED_H, 0) * 3600 / INTERVAL_S_1 THREADS_1,
      IFNULL(T2.ELAPSED_H, 0) * 3600 / INTERVAL_S_2 THREADS_2,
      T1.ROW_NUM ROW_NUM_1,
      T2.ROW_NUM ROW_NUM_2,
      BI.TOP_N_THREAD_APP_COMP_NAMES
    FROM
      BASIS_INFO BI,
      ( SELECT ROW_NUMBER () OVER ( ORDER BY SUM(ELAPSED_H) DESC ) ROW_NUM, APPLICATION_COMPONENT_NAME, SUM(ELAPSED_H) ELAPSED_H FROM THREADS_2 GROUP BY APPLICATION_COMPONENT_NAME ) T2 FULL OUTER JOIN
      ( SELECT ROW_NUMBER () OVER ( ORDER BY SUM(ELAPSED_H) DESC ) ROW_NUM, APPLICATION_COMPONENT_NAME, SUM(ELAPSED_H) ELAPSED_H FROM THREADS_1 GROUP BY APPLICATION_COMPONENT_NAME ) T1 ON
        T1.APPLICATION_COMPONENT_NAME = T2.APPLICATION_COMPONENT_NAME
  )
  WHERE
  ( ROW_NUM_1 <= TOP_N_THREAD_APP_COMP_NAMES OR ROW_NUM_2 <= TOP_N_THREAD_APP_COMP_NAMES )
  UNION ALL
  SELECT 
...
( SELECT 100 LINE_NO, 'SAP HANA DIFF REPORT' DETAIL, '' VALUE_1, '' VALUE_2, '' VALUE_3, '' VALUE_4 FROM DUMMY
  ...
  UNION ALL
  SELECT 110610, '', '', '', '', '' FROM DUMMY
  UNION ALL
  SELECT 110780, '', '', '', '', '' FROM DUMMY
  UNION ALL
  SELECT 110790, 'APPLICATION_COMPONENT_NAME', 'THREADS_1', 'THREADS_2', 'DIFF_THREADS', 'DIFF_PCT' FROM DUMMY
  UNION ALL
  SELECT 110795, '', '', '', '', '' FROM DUMMY
  UNION ALL
  SELECT 110890, '', '', '', '', '' FROM DUMMY
  UNION ALL
