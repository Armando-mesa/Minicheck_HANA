...
- HANA_Indexes_Overview_2.00.080+
...
- When columns of the index are not loaded, MOST_SELECTIVE_COLUMN_NAME and INVERTED_INDIVIDUAL_COST are empty / 0.
...
- 2024/05/02:  2.4 (MIN_MEM_CONC_GB added)
- 2024/07/18:  2.5 (dedicated 2.00.080+ version including persistent memory and page memory columns of M_CS_INDEXES)
...
- MIN_MEM_CONC_GB
  ...
  Minimum concat attribute memory size (GB)

  5               --> Only display index with a concat attribute memory size of at least 5 GB
  -1              --> No restriction related to concat attribute memory size
...
- MEM_PERS_GB:  Current persistent memory size of index in memory (GB)
- MEM_PAGE_GB:  Current index size in NSE buffer cache (GB)
- SEL_COL_NAME: Name of most selective column for inverted individual index (empty if not all index columns are loaded)
- INDIV_COSTS:  Inverted individual costs (0 if not all index columns are loaded)
...
*/
  ...
  LPAD(TO_DECIMAL(MEM_PERS_GB, 10, 2), 11) MEM_PERS_GB,
  LPAD(TO_DECIMAL(MEM_PAGE_GB, 10, 2), 11) MEM_PAGE_GB,
  SEL_COL_NAME,
...
( SELECT
    ...
    SUM(I.MEM_PERS_GB) MEM_PERS_GB,
    SUM(I.MEM_PAGE_GB) MEM_PAGE_GB,
    BI.ORDER_BY
  FROM
  ...
  ( SELECT
      ...
      SUM(MEM_PERS_GB) MEM_PERS_GB,
      SUM(MEM_PAGE_GB) MEM_PAGE_GB,
    ...
    ( SELECT
        ...
        ( CI.MEMORY_SIZE_IN_TOTAL + CI.MAIN_PERSISTENT_MEMORY_SIZE_IN_INDEX + CI.MAIN_PERSISTENT_MEMORY_SIZE_IN_CONCAT ) / 1024 / 1024 / 1024 MEM_TOT_GB,
        ( CI.MEMORY_SIZE_IN_CONCAT + CI.MAIN_PERSISTENT_MEMORY_SIZE_IN_CONCAT ) / 1024 / 1024 / 1024 MEM_CONC_GB,
        ( CI.MAIN_PERSISTENT_MEMORY_SIZE_IN_INDEX + CI.MAIN_PERSISTENT_MEMORY_SIZE_IN_CONCAT ) / 1024 / 1024 / 1024 MEM_PERS_GB,
        ( CI.MAIN_PAGE_LOADABLE_MEMORY_SIZE_IN_INDEX + CI.MAIN_PAGE_LOADABLE_MEMORY_SIZE_IN_CONCAT ) / 1024 / 1024 / 1024 MEM_PAGE_GB
      FROM
  ...
  WHERE
    ...
    ( BI.MIN_MEM_CONC_GB = -1 OR I.MEM_CONC_GB >= BI.MIN_MEM_CONC_GB ) AND
