...
- HANA_LOBs_DiskLOBs_2.00.040+

[DESCRIPTION]

- Overview of disk LOBs
...
- MIN_DISK_LOBS
  ...
  Minimum number of disk LOBs

  10000000        --> Only display lines representing at least 10 million disk LOBs
  -1              --> No restriction related to number of disk LOBs

- MAX_AVG_BIN_KB

  Maximum average binary size of disk LOB

  4               --> Only display lines with average binary disk LOB size of <= 4 KB
  -1              --> No restriction related to average binary disk LOB size
...
- LOB_TYPE:     LOB storage type (PACKED for packed LOBs, FILE for file LOBs)
- STORE:        Table store (ROW, COLUMN)
- NUM_LOBS:     Number of disk LOBs
...
- AVG_PHYS_KB:  Physical size per disk LOB (KB)
- AVG_BIN_KB:   Binary size per disk LOB (KB)
...
( SELECT
    ...
    CASE WHEN BI.AGGREGATE_BY = 'NONE' OR INSTR(BI.AGGREGATE_BY, 'LOB_TYPE') != 0 THEN L.LOB_STORAGE_TYPE                                                      ELSE MAP(BI.LOB_TYPE, '%', 'any', BI.LOB_TYPE)         END LOB_TYPE,
  ...
  ( SELECT                 /* Modification section */
      ...
      'NONE' AGGREGATE_BY,                 /* HOST, PORT, SERVICE, SCHEMA, TABLE, COLUMN, LOB_TYPE, STORE or comma separated combinations, NONE for no aggregation */
  ...
  GROUP BY
    ...
    CASE WHEN BI.AGGREGATE_BY = 'NONE' OR INSTR(BI.AGGREGATE_BY, 'LOB_TYPE') != 0 THEN L.LOB_STORAGE_TYPE                                                      ELSE MAP(BI.LOB_TYPE, '%', 'any', BI.LOB_TYPE)         END,
...
WHERE
  ( MIN_DISK_LOBS = -1 OR NUM >= MIN_DISK_LOBS ) AND
...
ORDER BY
  ...
  MAP(ORDER_BY, 'COUNT', NUM_LOBS, 'PHYS', PHYS_SIZE_MB, 'BIN', BIN_SIZE_MB, 'FRAG', FRAG_SIZE_MB, 'MEM', MEM_SIZE_MB, 'READS', READS) DESC