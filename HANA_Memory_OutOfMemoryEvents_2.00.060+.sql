...
- HANA_Memory_OutOfMemoryEvents_2.00.060+
...
- WORKLOAD_CLASS_NAME available with SAP HANA >= 2.0 SPS 06
...
- 2022/05/26:  1.3 (dedicated 2.00.060+ version including SITE_ID for data source HISTORY)
- 2024/03/07:  1.4 (WORKLOAD_CLASS_NAME added)
...
- WORKLOAD_CLASS
  ...
  'WKL_500'       --> Out of memory events related to workload class WKL_500
...
( SELECT
    CASE 
      ...
      ELSE 'any' 
    END OOM_TIME,
    CASE WHEN BI.AGGREGATE_BY = 'NONE' OR INSTR(BI.AGGREGATE_BY, 'SITE_ID')        != 0 THEN TO_VARCHAR(OE.SITE_ID) ELSE MAP(BI.SITE_ID,         -1, 'any', TO_VARCHAR(BI.SITE_ID))      END SITE_ID,
    ...
    CASE WHEN BI.AGGREGATE_BY = 'NONE' OR INSTR(BI.AGGREGATE_BY, 'WORKLOAD_CLASS') != 0 THEN OE.WORKLOAD_CLASS_NAME ELSE MAP(BI.WORKLOAD_CLASS, '%', 'any', BI.WORKLOAD_CLASS)           END WORKLOAD_CLASS,
  ...
  ( SELECT
    ...
    ( SELECT                              /* Modification section */
        ...
        '%' EVENT_REASON,
        'HISTORY' DATA_SOURCE,
        'TIME, WORKLOAD_CLASS, REASON' AGGREGATE_BY,         /* TIME, SITE_ID, HOST, PORT, CONN_ID, WORKLOAD_CLASS, HASH, REASON or comma separated combinations, NONE for no aggregation */
  ...
  WHERE
    ...
    ( BI.SITE_ID = -1 OR ( BI.SITE_ID = 0 AND OE.SITE_ID IN (-1, 0) ) OR OE.SITE_ID = BI.SITE_ID ) AND
    ...
    OE.WORKLOAD_CLASS_NAME LIKE BI.WORKLOAD_CLASS AND
  ...
  GROUP BY
    CASE 
      ...
      ELSE 'any' 
    END,
    CASE WHEN BI.AGGREGATE_BY = 'NONE' OR INSTR(BI.AGGREGATE_BY, 'SITE_ID')        != 0 THEN TO_VARCHAR(OE.SITE_ID) ELSE MAP(BI.SITE_ID,         -1, 'any', TO_VARCHAR(BI.SITE_ID))      END,
    ...
    CASE WHEN BI.AGGREGATE_BY = 'NONE' OR INSTR(BI.AGGREGATE_BY, 'WORKLOAD_CLASS') != 0 THEN OE.WORKLOAD_CLASS_NAME ELSE MAP(BI.WORKLOAD_CLASS, '%', 'any', BI.WORKLOAD_CLASS)           END,
