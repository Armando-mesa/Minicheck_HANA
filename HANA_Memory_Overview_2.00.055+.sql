...
- M_SERVICES_MEMORY.FAST_RESTART_MEMORY_FILE_SIZE available with SAP HANA >= 2.00.055
- M_HOST_RESOURCE_UTILIZATION.USED_PHYSICAL_MEMORY may not properly consider PMEM / FRO (bug 327308).
...
- 2023/07/09:  2.2 (service allocation limit and system ID / DB name / execution time added)
- 2023/09/07:  2.3 (top NSE tables added)
- 2023/10/14:  2.4 ("Local database memory (used)" added)
- 2023/11/09:  2.5 (dedicated 2.00.055+ version including FAST_RESTART_MEMORY_FILE_SIZE)
...
|                                    |         |                                 |                                                                      |
|Physical memory                     |      256|      256 (saphana001)           |                                                                      |
|                                    |         |                                 |                                                                      |
|HANA instance memory (allocated)    |      113|      113 (saphana001)           |                                                                      |
|                                    |         |                                 |                                                                      |
|HANA instance memory (used)         |       85|       85 (saphana001)           |                                                                      |
|                                    |         |                                 |                                                                      |
|HANA shared memory                  |       23|       23 (saphana001)           |                                                                      |
|                                    |         |                                 |                                                                      |
|HANA heap memory (used)             |       52|       52 (saphana001)           |       10 (Pool/NameIdMapping/RoDict)                                 |
...
|                                    |         |                                 |                                                                      |
|Column store size                   |       19|       19 (saphana001)           |        3 (WBCROSSGT)                                                 |
|                                    |         |                                 |        2 (BALDAT)                                                    |
|                                    |         |                                 |        1 (DBTABLOG)                                                  |
|                                    |         |                                 |        1 (DOKTL)                                                     |
...
|                                    |         |                                 |        1 (REPOSRC)                                                   |
|                                    |         |                                 |                                                                      |
|Row store size                      |       30|       26 (saphana001)           |        5 (D010TAB)                                                   |
|                                    |         |                                 |        3 (D010INC)                                                   |
|                                    |         |                                 |        2 (REPOSRC)                                                   |
|                                    |         |                                 |        2 (INDX)                                                      |
|                                    |         |                                 |        2 (DD03L)                                                     |
...
( SELECT
    100 LINE_NO,
    'System ID / DB name:'  NAME,
    '' TOTAL_GB,
    SYSTEM_ID || CHAR(32) || '/' || CHAR(32) || DATABASE_NAME DETAIL_GB,
    '' DETAIL2_GB
  FROM
    M_DATABASE
  UNION ALL
  ...
  FROM
    DUMMY
  UNION ALL ( SELECT 300, '', '', '', '' FROM DUMMY )
  UNION ALL
  SELECT
  1000,
    'License memory limit',
    LPAD(TO_DECIMAL(ROUND(PRODUCT_LIMIT), 10, 0), 8),
    ' ',
    ' '
  FROM
    M_LICENSE
...
*/
  ...
  FROM
    M_HOST_RESOURCE_UTILIZATION
  UNION ALL ( SELECT TOP 1 5490, ' ', ' ', ' ', ' ' FROM M_CONFIGURATION_PARAMETER_VALUES WHERE FILE_NAME = 'global.ini' AND SECTION = 'memorymanager' AND KEY = 'allocationlimit' AND VALUE != '' )
  UNION ALL 
  SELECT
    5500 + ROW_NUMBER() OVER (ORDER BY S.HOST),
    MAP(ROW_NUMBER () OVER (ORDER BY S.HOST), 1, 'Service allocation limit', ' ') NAME,    
    MAP(ROW_NUMBER () OVER (ORDER BY S.HOST), 1, LPAD(TO_DECIMAL(MAX(S.ALLOCATION_LIMIT) / 1024 / 1024 / 1024, 10, 0), 8), ' ') TOTAL_GB,
    LPAD(TO_DECIMAL(MAX(S.ALLOCATION_LIMIT) / 1024 / 1024 / 1024, 10, 0), 6) || ' (' || S.HOST || ')' DETAIL_GB,
    ' ' DETAIL2_GB
  FROM
    M_DATABASE D,
  ...
  WHERE
    ...
    ( SELECT
      ...
      WHERE
        FILE_NAME IN ( 'global.ini', 'indexserver.ini' ) AND
        SECTION = 'memorymanager' AND
        KEY = 'allocationlimit' AND
        VALUE != ''
    )
  GROUP BY
    S.HOST
  ...
  FROM
    M_HOST_RESOURCE_UTILIZATION
  UNION ALL ( SELECT 8490, ' ', ' ', ' ', ' ' FROM DUMMY )
  UNION ALL
  SELECT
    8500 + ROW_NUMBER () OVER (ORDER BY SM.HOST),
    MAP(ROW_NUMBER () OVER (ORDER BY SM.HOST), 1, 'HANA local database memory (used)', ' '),
    MAP(ROW_NUMBER () OVER (ORDER BY SM.HOST), 1, LPAD(TO_DECIMAL(ROUND(SUM(SUM(SM.STACK_SIZE + SM.HEAP_MEMORY_USED_SIZE + SM.SHARED_MEMORY_USED_SIZE + SM.FAST_RESTART_MEMORY_FILE_SIZE + 
      SM.PERSISTENT_MEMORY_FILE_SIZE ) + MAX(SM.CODE_SIZE)) OVER () / 1024 / 1024 / 1024), 10, 0), 8), ' '),
    LPAD(TO_DECIMAL(ROUND((SUM(SM.STACK_SIZE + SM.HEAP_MEMORY_USED_SIZE + SM.SHARED_MEMORY_USED_SIZE + SM.FAST_RESTART_MEMORY_FILE_SIZE +     
      SM.PERSISTENT_MEMORY_FILE_SIZE ) + MAX(SM.CODE_SIZE)) / 1024 / 1024 / 1024), 10, 0), 6) || ' (' || SM.HOST || ')',
    ' '
  FROM
    M_SERVICE_MEMORY SM,
    M_SERVICES S
  ...
  UNION ALL
  ( SELECT
      9491 + ROW_NUMBER () OVER (ORDER BY HOST) / 1000,
      MAP(ROW_NUMBER () OVER (ORDER BY HOST), 1, 'HANA free segment list', ' '),
      MAP(ROW_NUMBER () OVER (ORDER BY HOST), 1, LPAD(TO_DECIMAL(ROUND(SUM(FSL_BYTE) OVER () / 1024 / 1024 / 1024), 10, 0), 8), ' '),
      LPAD(TO_DECIMAL(ROUND(FSL_BYTE / 1024 / 1024 / 1024), 10, 0), 6) || ' (' || HOST || ')',
      ' '
    FROM
    ( SELECT
        HOST,
        SUM(TOTAL_SIZE) FSL_BYTE
      FROM
        M_HEAP_MEMORY_AREAS
      WHERE
        AREA = 'FreeSegmentList'
      GROUP BY
        HOST
      HAVING
        SUM(TOTAL_SIZE) / 1024 / 1024 / 1024 > 10
  ...
  UNION ALL ( SELECT TOP 1 9495, ' ', ' ', ' ', ' ' FROM M_HEAP_MEMORY_AREAS WHERE AREA = 'FreeSegmentList' GROUP BY HOST HAVING SUM(TOTAL_SIZE) > 10 * 1024 * 1024 * 1024 )
  UNION ALL
  SELECT
    10000 + LINE_NO / 1000,
    NAME,
  ...
  WHERE
    USED_GB > 0 AND
    ROW_NUM <= 10
  UNION ALL ( SELECT TOP 1 10490, ' ', ' ', ' ', ' ' FROM M_CACHES WHERE CACHE_ID NOT IN ( 'L/Code', 'L/Interpret' ) GROUP BY HOST, CACHE_ID HAVING ROUND(SUM(USED_SIZE / 1024 / 1024 / 1024)) > 0 )
  UNION ALL
  SELECT
    10500 + LINE_NO / 1000,
    NAME,
  ...
  UNION ALL ( SELECT TOP 1 10690, ' ', ' ', ' ', ' ' FROM M_ACTIVE_STATEMENTS GROUP BY HOST, STATEMENT_HASH HAVING SUM(ALLOCATED_MEMORY_SIZE) / 1024 / 1024 / 1024 > 1 )
  UNION ALL
  SELECT
    10700 + LINE_NO / 1000,
    NAME,
  ...
  FROM
  ( SELECT
      ROW_NUMBER () OVER (ORDER BY HOST, ALLOCATED_MEMORY_SIZE DESC) LINE_NO,
      MAP(ROW_NUMBER () OVER (ORDER BY HOST, ALLOCATED_MEMORY_SIZE DESC), 1, 'Intermediate results', ' ') NAME,
      MAP(ROW_NUMBER () OVER (ORDER BY HOST, ALLOCATED_MEMORY_SIZE DESC), 1, LPAD(TO_DECIMAL(ROUND(SUM(ALLOCATED_MEMORY_SIZE) OVER () / 1024 / 1024 / 1024), 10, 0), 8), ' ') TOTAL_GB,
      MAP(ROW_NUMBER () OVER (PARTITION BY HOST ORDER BY ALLOCATED_MEMORY_SIZE DESC), 1, 
        LPAD(TO_DECIMAL(ROUND(SUM(ALLOCATED_MEMORY_SIZE) OVER (PARTITION BY HOST) / 1024 / 1024 / 1024), 10, 0), 6) || ' (' || HOST || ')', ' ') DETAIL_GB,
      LPAD(TO_DECIMAL(ROUND(SUM(ALLOCATED_MEMORY_SIZE) / 1024 / 1024 / 1024), 10, 0), 6) || ' (' || STATEMENT_HASH || ')' DETAIL2_GB,
      ROW_NUMBER () OVER (PARTITION BY HOST ORDER BY ALLOCATED_MEMORY_SIZE DESC) ROW_NUM
    FROM
    ( SELECT
        HOST,
        SUM(ALLOCATED_MEMORY_SIZE) ALLOCATED_MEMORY_SIZE,
        STATEMENT_HASH
      FROM
        M_ACTIVE_STATEMENTS
      GROUP BY
        ...
        STATEMENT_HASH
      HAVING
        SUM(ALLOCATED_MEMORY_SIZE) / 1024 / 1024 / 1024 > 1
    )
    GROUP BY
      HOST,
  ...
  WHERE
    ROW_NUM <= 10
  UNION ALL ( SELECT TOP 1 10790, ' ', ' ', ' ', ' ' FROM M_PLE_RUNTIME_OBJECTS GROUP BY HOST, OBJECT_TYPE HAVING ROUND(SUM(MEMORY_SIZE / 1024 / 1024 / 1024)) > 0 )
  UNION ALL
  SELECT
    10800 + LINE_NO / 1000,
    NAME,
  ...
  WHERE
    TABLE_SIZE_GB > 0 AND
    ROW_NUM <= 10
  UNION ALL ( SELECT TOP 1 12490, ' ', ' ', ' ', ' ' FROM M_CS_TABLES GROUP BY HOST, TABLE_NAME HAVING SUM(MEMORY_SIZE_IN_PAGE_LOADABLE_MAIN) >= 1024 * 1024 * 1024 )
  UNION ALL
  SELECT
    12500 + LINE_NO / 1000,
    NAME,
  ...
  ( SELECT
      ROW_NUMBER () OVER (ORDER BY HOST, SUM(MEMORY_SIZE_IN_TOTAL) DESC) / 10000 LINE_NO,
      MAP(ROW_NUMBER () OVER (ORDER BY HOST, MEMORY_SIZE_IN_TOTAL DESC), 1, 'NSE table size', ' ') NAME,
      MAP(ROW_NUMBER () OVER (ORDER BY HOST, MEMORY_SIZE_IN_TOTAL DESC), 1, LPAD(TO_DECIMAL(ROUND(SUM(MEMORY_SIZE_IN_TOTAL) OVER () / 1024 / 1024 / 1024), 10, 0), 8), ' ') TOTAL_GB,
      MAP(ROW_NUMBER () OVER (PARTITION BY HOST ORDER BY MEMORY_SIZE_IN_TOTAL DESC), 1, 
        LPAD(TO_DECIMAL(ROUND(SUM(MEMORY_SIZE_IN_TOTAL) OVER (PARTITION BY HOST) / 1024 / 1024 / 1024), 10, 0), 6) || ' (' || HOST || ')', ' ') DETAIL_GB,
      LPAD(TO_DECIMAL(ROUND(SUM(MEMORY_SIZE_IN_TOTAL) / 1024 / 1024 / 1024), 10, 0), 6) || ' (' || TABLE_NAME || ')' DETAIL2_GB,
    ...
    ( SELECT
        HOST,
        TABLE_NAME,
        SUM(MEMORY_SIZE_IN_PAGE_LOADABLE_MAIN) MEMORY_SIZE_IN_TOTAL
      FROM
        M_CS_TABLES
      GROUP BY
        HOST,
        TABLE_NAME
      HAVING
        SUM(MEMORY_SIZE_IN_PAGE_LOADABLE_MAIN) > 0
    )
    GROUP BY
      HOST,
      TABLE_NAME,
