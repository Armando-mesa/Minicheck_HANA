...
- HANA_Memory_TopConsumers_2.00.060+
...
- Column SITE_ID in statistics server table histories contains also remote sites starting with SAP HANA 2.00.060
...
- 2022/05/26:  2.6 (dedicated 2.00.060+ version considering SITE_ID in table histories)
- 2023/12/21:  2.7 (EXCLUDE_HEAP_FRAGMENTATION included)
...
( SELECT
  ...
  ( SELECT                                                      /* Modification section */
      ...
      ' ' ONLY_PERSISTENT_MEMORY_AREAS,
      'CURRENT' DATA_SOURCE,
      'DETAIL' AGGREGATE_BY,        /* TIME, SCHEMA, DETAIL, SITE_ID, HOST, PORT, MAINAREA, SUBAREA or comma separated combinations, NONE for no aggregation */
...
( SELECT
  ...
  ( SELECT
      ...
      CASE WHEN BI.AGGREGATE_BY = 'NONE' OR INSTR(BI.AGGREGATE_BY, 'HOST')       != 0 THEN TO_VARCHAR(D.SITE_ID)                        ELSE MAP(BI.SITE_ID,      -1, 'any', TO_VARCHAR(BI.SITE_ID)) END SITE_ID,
  ...
  WHERE
    ...
    GROUP BY
      ...
      CASE WHEN BI.AGGREGATE_BY = 'NONE' OR INSTR(BI.AGGREGATE_BY, 'HOST')       != 0 THEN TO_VARCHAR(D.SITE_ID)                        ELSE MAP(BI.SITE_ID,      -1, 'any', TO_VARCHAR(BI.SITE_ID)) END,
