...
- 2023/10/13:  2.4 (CNT added)
- 2024/03/18:  2.5 (ORDER_BY added)
...
- ORDER_BY
  ...
  'REPLICATION'   --> Sorting by system replication type / target
...
- LOG_H:         Time required for log shipping (h), can be higher than absolute time because multiple log buffers can be in shipping state at the same time
  ...
  ( SELECT
    ...
    ( SELECT
      ...
      ( SELECT                                                     /* Modification section */
          ...
          'REPLICATION' ORDER_BY       /* TIME, REPLICATION */
        FROM
          DUMMY
      )
    ) BI,
...
ORDER BY
  MAP(ORDER_BY, 'REPLICATION', SITE_ID || HOST || SECONDARY_HOST || PORT || REP_MODE),
  SNAPSHOT_TIME DESC,
  SITE_ID,
  HOST,
  SECONDARY_HOST,
