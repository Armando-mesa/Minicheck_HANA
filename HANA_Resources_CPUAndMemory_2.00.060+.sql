...
- INSTANCE_TOTAL_MEMORY_USED_SIZE available as of revision 1.00.70
- CPU figures for CURRENT are always the average numbers since startup
...
- 2023/08/30:  1.9 (MIN_SYSTEM_CPU_UTILIZATION_PCT included)
- 2024/02/25:  2.0 (DATA_SOURCE = 'CURRENT' included)
...
- MIN_CPU_UTILIZATION_PCT
  ...
  Threshold for minimum (user + system) CPU utilization (%)

  20              --> Restrict the output to snapshots with a CPU utilization of more than 20 %
  -1              --> No restriction based on CPU utilization

- MIN_SYSTEM_CPU_UTILIZATION_PCT

  Threshold for minimum system CPU utilization (%)

  20              --> Restrict the output to snapshots with a system CPU utilization of more than 20 %
  -1              --> No restriction based on system CPU utilization

- EXCLUDE_STANDBY

  Possibility to exclude standby nodes
...
- DATA_SOURCE
  ...
  'CURRENT'       --> Data from memory information
...
- CPU_PCT:       CPU usage (%, user + system)
- CPU_SYS_PCT:   System CPU usage (%)
- CPU_USER:      Average number of CPU threads busy with user activities
- CPU_SYS:       Average number of CPU threads busy with system activities
- CPU_IOWAIT:    Average number of CPU threads waiting for I/O
- CPU_IDLE:      Average number of CPU threads being idle
  ...
  ( SELECT
    ...
    ( SELECT
      ...
      ( SELECT                                 /* Modification section */
          ...
          'X' EXCLUDE_STANDBY,
          'HISTORY' DATA_SOURCE,
          'AVG' AGGREGATION_TYPE,           /* AVG, PEAK */
          'NONE' AGGREGATE_BY,           /* SITE_ID, HOST, TIME or comma separated list, NONE for no aggregation */
