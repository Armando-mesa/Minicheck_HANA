...
- HANA_SQL_PlanStability_StatementHints_2.00.050+
...
- Statement hints available starting with SAP HANA 1.00.122.03
- STATEMENT_HINTS.STATEMENT_HASH available starting with SAP HANA 2.00.040
- STATEMENT_HINTS.COMMENTS available starting with SAP HANA 2.00.050
...
- 2023/08/26:  1.4 (dedicated 2.00.050+ version including COMMENTS)
...
- COMMENTS:           Comments (provided via COMMENT option when creating statement hint)
...
*/
  ...
  MAP(SH.STATEMENT_STRING, '', SH.COMMENTS, MAP(ROW_NUMBER () OVER (PARTITION BY SH.STATEMENT_STRING ORDER BY O.LINE_NO), 1, SH.COMMENTS, '')) COMMENTS
FROM
( SELECT
  ...
  ( SELECT DISTINCT
      TO_VARCHAR(CASE WHEN SH.STATEMENT_STRING LIKE '' THEN IFNULL(C.STATEMENT_STRING, '') ELSE SH.STATEMENT_STRING END) STATEMENT_STRING,
      SH.STATEMENT_HASH,
      SH.HINT_STRING,
  ...
  WHERE
    ...
    SH.COMMENTS LIKE BI.COMMENTS AND
    ( BI.EXCLUDE_PREDEFINED_ENTRIES = ' ' OR SH.LAST_ENABLE_USER != 'SYS' ) AND
    ( BI.EXCLUDE_DELIVERED_ENTRIES = ' ' OR SH.COMMENTS NOT LIKE 'SAP Note 2700051%' ) AND
