...
- TOTAL_BUFFER_CACHE_PAGE_MISS_COUNT and TOTAL_BUFFER_CACHE_IO_READ_SIZE available with SAP HANA >= 2.00.070
...
- 2023/04/23:  4.6 (dedicated 2.00.070+ version including TOTAL_BUFFER_CACHE* columns)
- 2024/01/10:  4.7 (significant redesign of output columns)
- 2024/07/24:  4.8 (dedicated 2.00.080+ version including HEX_REJECTION_REASON)
...
- HEX_REJECTION_REASON

  HEX engine rejection reason

  'FALLBACK_USED option is given' --> Only display entries with HEX rejection reason 'FALLBACK_USED option is given'
  '%'                             --> No restriction related to HEX rejection reason
...
- S:                Store (R for row store tables, C for column store tables)
- P:                'X' in case of pinned plan or statement hint, otherwise ' '
- I:                'X' in case of internal statement, otherwise ' '
- D:                'X' in case of distributed request (accessing more than 1 node), ' ' in case of single node request 
- ENG:              Execution engines (C -> column, E -> ESX, H -> HEX, O -> OLAP, R -> Row, S -> SQLScript, X -> external)
...
- AVG_BC_IO_MB:     Average NSE buffer cache I/O read size per execution (MB)
- PLAN_MB:          Execution plan memory size (MB)
- BC_MISS_COUNT:    Total number of NSE buffer cache page misses
...
( SELECT
  ...
  ( SELECT                                                      /* Modification section */
      ...
      'CURRENT' DATA_SOURCE,     /* CURRENT, HISTORY, RESET, STATISTICS */
      'HEX_REJECTION_REASON' AGGREGATE_BY,       /* TIME, HOST, PORT, HASH, SQL_TYPE, PLAN_ID, STORE, INV_REASON, HEX_REJECTION__REASON, SHARING_TYPE, DIST, SCHEMA, LOCATION, INTERNAL, TABLES, APP_NAME, APP_SOURCE, PINNED, ENGINES or comma separated list, NONE for no aggregation */
      'NONE' TIME_AGGREGATE_BY,   /* HOUR, DAY, HOUR_OF_DAY or database time pattern, TS<seconds> for time slice, NONE for no aggregation */
      'TIME' ORDER_BY,        /* ELAPSED, EXEC, PREP, CURSOR, OPEN, FETCH, LOCK, EXECUTIONS, RECORDS, NETWORK_COUNT, NETWORK_SIZE, NETWORK_TIME, MEMORY, MEM_PER_EXEC, PLAN_MEM, THREAD, BC_MISS_COUNT, BC_IO_SIZE */
      -1 RESULT_ROWS
    FROM
      DUMMY
  ) BI
...
( SELECT
  ...
  ( SELECT
      ...
      CASE WHEN AGGREGATE_BY = 'NONE' OR INSTR(AGGREGATE_BY, 'HEX_REJECTION_REASON') != 0 THEN HEX_REJECTION_REASON ELSE MAP(BI_HEX_REJECTION_REASON, '%', 'any', BI_HEX_REJECTION_REASON) END HEX_REJECTION_REASON,
      COUNT(*) NUM,
      ...
      CASE ORDER_BY
        ...
        WHEN 'BC_MISS_COUNT' THEN SUM(BC_MISS_COUNT)
        WHEN 'BC_IO_SIZE'    THEN SUM(TOTAL_BUFFER_CACHE_IO_READ_SIZE_MB)
    ...
    ( SELECT
      ...
      ( SELECT
          ...
          BI.HEX_REJECTION_REASON BI_HEX_REJECTION_REASON,
        ...
        ( SELECT
            ...
            MAX(HEX_REJECTION_REASON) HEX_REJECTION_REASON,
            MAX(PLAN_MEMORY_SIZE) PLAN_MEMORY_SIZE
          FROM
            SQL_CACHE C
          GROUP BY
    ...
    GROUP BY
      ...
      CASE WHEN AGGREGATE_BY = 'NONE' OR INSTR(AGGREGATE_BY, 'HEX_REJECTION_REASON') != 0 THEN HEX_REJECTION_REASON ELSE MAP(BI_HEX_REJECTION_REASON, '%', 'any', BI_HEX_REJECTION_REASON) END,
      ORDER_BY,
      RESULT_ROWS
  )
)
