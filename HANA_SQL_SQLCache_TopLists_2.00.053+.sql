...
- HANA_SQL_SQLCache_TopLists_2.00.053+
...
- 2024/04/08:  2.3 (replacement of ELAPSED TIME with EXECUTION TIME)
- 2024/04/11:  2.4 (TABLE_NAMES included)
...
- E:              Used execution engines (C -> column, E -> ESX, H -> HEX, O -> OLAP, R -> row, S -> SQLScript, X -> externa)
...
( SELECT
  ...
  ( SELECT
      ...
      CASE
        ...
        WHEN MAX(APPLICATION_NAME) = 'Mashup Engine'                THEN 'ME'
        ...
        ELSE                                                             'UN'
      END SQL_ORIGIN,
      MAX(CASE WHEN C.ENGINES LIKE '%COLUMN%'      THEN 'C' ELSE '' END) || MAX(CASE WHEN C.ENGINES LIKE '%EXTERNAL%' THEN 'X' ELSE '' END) ||
        MAX(CASE WHEN C.ENGINES LIKE '%ESX%'       THEN 'E' ELSE '' END) || MAX(CASE WHEN C.ENGINES LIKE '%HEX%'      THEN 'H' ELSE '' END) || 
        MAX(CASE WHEN C.ENGINES LIKE '%OLAP%'      THEN 'O' ELSE '' END) || MAX(CASE WHEN C.ENGINES LIKE '%ROW%'      THEN 'R' ELSE '' END) ||
        MAX(CASE WHEN C.ENGINES LIKE '%SQLSCRIPT%' THEN 'S' ELSE '' END) ENGINES,
