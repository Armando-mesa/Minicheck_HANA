...
- HANA_SQL_StatementHash_DiffReport_2.00.053+
...
- 2023/06/21:  1.5 (dedicated 2.00.053+ version including EXECUTION_ENGINE)
...
( SELECT
  ...
  ( SELECT
    ...
    ( SELECT                        /* Modification section */
        'C-D31' BEGIN_TIME_1,                  /* YYYY/MM/DD HH24:MI:SS timestamp, C, C-S<seconds>, C-M<minutes>, C-H<hours>, C-D<days>, C-W<weeks>, E-S<seconds>, E-M<minutes>, E-H<hours>, E-D<days>, E-W<weeks>, MIN */
        ...
        'ed75dd398c0290d6f09294e342bdba0b' STATEMENT_HASH_1,
...
( SELECT  10 LINE_NO, 'Statement hash' KEY_FIGURE, ' ' CALCULATIONS FROM DUMMY UNION ALL
  SELECT  15, 'Execution engines',     ' ' FROM DUMMY UNION ALL
  ...
  SELECT  70, 'Execution time (ms)',   'X' FROM DUMMY UNION ALL
  SELECT  80, 'Trans. lock time (ms)', 'X' FROM DUMMY UNION ALL
  SELECT  90, 'Called threads',        'X' FROM DUMMY UNION ALL
  SELECT 100, 'Used memory (MB)',      'X' FROM DUMMY UNION ALL
  SELECT 110, 'Cursor time (ms)',      'X' FROM DUMMY UNION ALL
  SELECT 120, 'Preparation time (ms)', 'X' FROM DUMMY
),
...
( SELECT
    ...
    RTRIM(
      MAX(CASE WHEN S.EXECUTION_ENGINE LIKE '%COLUMN%'    THEN 'COLUMN,'    || CHAR(32) ELSE '' END) ||
      MAX(CASE WHEN S.EXECUTION_ENGINE LIKE '%ESX%'       THEN 'ESX,'       || CHAR(32) ELSE '' END) ||
      MAX(CASE WHEN S.EXECUTION_ENGINE LIKE '%EXTERNAL%'  THEN 'EXTERNAL,'  || CHAR(32) ELSE '' END) ||
      MAX(CASE WHEN S.EXECUTION_ENGINE LIKE '%HEX%'       THEN 'HEX,'       || CHAR(32) ELSE '' END) ||
      MAX(CASE WHEN S.EXECUTION_ENGINE LIKE '%OLAP%'      THEN 'OLAP,'      || CHAR(32) ELSE '' END) ||
      MAX(CASE WHEN S.EXECUTION_ENGINE LIKE '%ROW%'       THEN 'ROW,'       || CHAR(32) ELSE '' END) ||
      MAX(CASE WHEN S.EXECUTION_ENGINE LIKE '%SQLSCRIPT%' THEN 'SQLSCRIPT,' || CHAR(32) ELSE '' END) , ',' || CHAR(32)) ENGINES
  FROM
    BASIS_INFO BI,
    M_SQL_PLAN_CACHE S
  WHERE
  ...
  ( SELECT
    ...
    ( SELECT
        ...
        CASE F.KEY_FIGURE
          ...
          WHEN 'Execution engines'     THEN S1.ENGINES
        ...
        CASE F.KEY_FIGURE
          ...
          WHEN 'Execution engines'     THEN S2.ENGINES
      ...
      FROM
        ...
        SQL_2 S2
      WHERE
        NOT ( F.KEY_FIGURE = 'Table types'           AND S1.STORE = S2.STORE ) AND
        NOT ( F.KEY_FIGURE = 'Sharing type'          AND S1.SHARING_TYPE = S2.SHARING_TYPE ) AND
        NOT ( F.KEY_FIGURE = 'Distributed'           AND S1.IS_DISTRIBUTED_EXECUTION = S2.IS_DISTRIBUTED_EXECUTION ) AND
        NOT ( F.KEY_FIGURE = 'Trans. lock time (ms)' AND S1.TOTAL_LOCK_WAIT_DURATION = 0 AND S2.TOTAL_LOCK_WAIT_DURATION = 0 ) AND
        NOT ( F.KEY_FIGURE = 'Called threads'        AND S1.TOTAL_CALLED_THREAD_COUNT = 0 AND S2.TOTAL_CALLED_THREAD_COUNT = 0 )
    ) F, 
