...
- 2024/06/10:  1.5 (ONLY_UNEXPECTED_INACTIVE_ACTIONS included)
...
- ONLY_UNEXPECTED_INACTIVE_ACTIONS

  Possibility to restrict the output to actions that are unexpectedly shown with Inactive state
  (only relevant for tenants, not for system DB)

  'X'             --> Only show actions that are unexpectedly shown with Inactive state
  ' '             --> No restriction to actions with unexpectedly shown Inactive states
...
*/
  ...
  SO.NAME,
  SO.TYPE,
  IFNULL(SS.EXTENSION_TYPE, '') EXT_TYPE,
...
( SELECT                                  /* Modification section */
    ...
    ' ' ONLY_UNKNOWN_STATES,
    ' ' ONLY_DELAYED_RUNS,
    'X' ONLY_UNEXPECTED_INACTIVE_ACTIONS,       /* only relevant for tenants, not for system DB */
  ...
  _SYS_STATISTICS.STATISTICS_SCHEDULE SS ON
    ...
    IFNULL(SS.EXTENSION_TYPE, '') LIKE BI.EXTENSION_TYPE AND
    ( BI.ONLY_UNKNOWN_STATES = ' ' OR SS.STATUS NOT IN ( 'Disabled', 'Idle', 'Inactive', 'Scheduled' ) ) AND
    ( BI.ONLY_UNEXPECTED_INACTIVE_ACTIONS = ' ' OR
      SS.STATUS = 'Inactive' AND SS.ID NOT IN ( 1, 20, 41, 58, 73, 74, 77, 83, 95, 96, 98, 117, 127, 5008, 5024, 5025, 5033, 5035, 5047, 5059 )
    ) INNER JOIN
