...
- 2024/09/11:  2.9 (consideration of multiple USED_FOR objects, MIN_TOTAL_DISK_SIZE_MB included)
...
- MIN_TOTAL_DISK_SIZE_MB

  Minimum total (i.e. final) disk size (MB)

  10              --> Only consider results with a total disk size of at least 10 MB
  -1              --> No restriction related to minimum total disk size
  ...
  ( SELECT
    ...
    ( SELECT
      ...
      ( SELECT                                      /* Modification section */
          ...
          -1 NUMA_NODE,
          -1 MIN_MEM_SIZE_MB,
          -1 MIN_TOTAL_MEM_SIZE_MB,
          1024 MIN_TOTAL_DISK_SIZE_MB,
          ' ' ONLY_INTERNAL_COLUMNS,
          'X' EXCLUDE_PK_AND_UNIQUE,
          ' ' ONLY_PK_AND_UNIQUE,
          'X' ONLY_UNUSED_CONCAT_ATTRIBUTES,
          ' ' GENERATE_DROP_CONCAT_ATT_COMMAND,
      ...
      ( SELECT
        ...
        FROM
          ...
          ( SELECT
              ...
              TABLE_NAME,
              COLUMN_NAME,
              STRING_AGG(USED_FOR, ' / ' ORDER BY USED_FOR) USED_FOR,
...
( MIN_TOTAL_DISK_SIZE_MB = -1 OR DISK_MB >= MIN_TOTAL_DISK_SIZE_MB ) AND
