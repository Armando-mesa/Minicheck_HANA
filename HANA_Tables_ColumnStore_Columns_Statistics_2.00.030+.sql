...
- 2023/11/28:  1.6 (LOAD_UNIT added)
...
- LOAD_UNIT:       Load unit (COLUMN, PAGE)
...
*/
  ...
  TABLE_NAME,
  COLUMN_NAME,
  LPAD(SCANNED_RECORDS, 18) SCANNED_RECORDS,
...
( SELECT
 ...
 /* MAX(SECONDS_BETWEEN(IFNULL(C.LAST_LOAD_TIME, SS.START_TIME), CURRENT_TIMESTAMP)) TIMEFRAME_S, */
    ...
    MAP(MIN(C.LOAD_UNIT), MAX(C.LOAD_UNIT), MIN(C.LOAD_UNIT), 'various') LOAD_UNIT,
    AVG(C.COUNT) RECORDS,
