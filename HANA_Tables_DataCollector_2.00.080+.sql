...
- HANA_Tables_DataCollector_2.00.080+
...
- SITE_ID in histories available with SAP HANA >= 2.00.060
- MAIN_PERSISTENT_MEMORY* and MAIN_PAGE_LOADABLE_MEMORY* columns in M_CS_INDEXES available with SAP HANA >= 2.00.080
...
- 2022/05/22:  1.5 (dedicated 2.00.060+ version including SITE_ID to restrict to primary site)
- 2022/08/21:  1.6 (M_EXECUTED_STATEMENTS included)
- 2023/11/25:  1.7 (LOAD_UNIT information included)
- 2024/05/24:  1.8 (lock and wait times for table optimizations included)
- 2024/07/18:  1.9 (dedicated 2.00.080+ version including persistent memory and page memory columns of M_CS_INDEXES)
...
( SELECT
  ...
  ( SELECT
    ...
    ( SELECT
      ...
      WHERE
        P.SITE_ID IN (-1, 0, CURRENT_SITE_ID()) AND
  ...
  ( SELECT
    ...
    ( SELECT
      ...
      WHERE
        M.SITE_ID IN (-1, 0, CURRENT_SITE_ID()) AND
...
( SELECT
  ...
  ( SELECT
      ...
      IFNULL(TO_DECIMAL(SUM(CS.INDEX_LOOKUP_COUNT) / S.TIMEFRAME_S, 18, 0), 0) INDEX_LOOKUPS_PER_S,
...
( SELECT
    ...
    CASE 
      ...
      ELSE I.INDEX_TYPE 
    END TY,
    TO_DECIMAL((I.MEMORY_SIZE_IN_TOTAL + I.MAIN_PERSISTENT_MEMORY_SIZE_IN_INDEX + I.MAIN_PERSISTENT_MEMORY_SIZE_IN_CONCAT) / 1024 / 1024, 10, 2) MEM_TOT_MB,
    TO_DECIMAL((I.MEMORY_SIZE_IN_CONCAT + I.MAIN_PERSISTENT_MEMORY_SIZE_IN_CONCAT) / 1024 / 1024, 10, 2) MEM_CONC_MB,
    TO_DECIMAL((I.MAIN_PERSISTENT_MEMORY_SIZE_IN_INDEX + I.MAIN_PERSISTENT_MEMORY_SIZE_IN_CONCAT) / 1024 / 1024, 10, 2) MEM_PERS_MB,
    TO_DECIMAL((I.MAIN_PAGE_LOADABLE_MEMORY_SIZE_IN_INDEX + I.MAIN_PAGE_LOADABLE_MEMORY_SIZE_IN_CONCAT) / 1024 / 1024, 10, 2) MEM_PAGE_MB,
...
( SELECT
    ...
    GREATEST(10, MAX(LENGTH(IC.TABLE_NAME)) OVER ()) TABLE_LEN,
...
( SELECT   10 LINE_NO, '*********************************' LINE                                                               FROM DUMMY UNION ALL
  ...
  SELECT TOP 1 4009, RPAD('SCHEMA_NAME', SCHEMA_LEN) || CHAR(32) || RPAD('TABLE_NAME', TABLE_LEN) || CHAR(32) || RPAD('TYPE', 6) || CHAR(32) ||
    LPAD('PARTS', 6) || CHAR(32) || LPAD('RECORDS', 11) || CHAR(32) || LPAD('MEM_TOTAL_GB', 12) || CHAR(32) || LPAD('MEM_DELTA_GB', 12) || LPAD('MEM_PAGE_GB', 12) || LPAD('CURR_DISK_GB', 13) || 
  ...
  SELECT TOP 1 4010, RPAD('=', SCHEMA_LEN, '=') || CHAR(32) || RPAD('=', TABLE_LEN, '=') || CHAR(32) || RPAD('=', 6, '=') || CHAR(32) ||
    LPAD('=', 6, '=') || CHAR(32) || LPAD('=', 11, '=') || CHAR(32) || LPAD('=', 12, '=') || CHAR(32) || LPAD('=', 12, '=') || CHAR(32) || LPAD('=', 11, '=') || CHAR(32) || LPAD('=', 12, '=') || CHAR(32) ||
  ...
  SELECT 4010 + ROW_NUMBER() OVER (ORDER BY SCHEMA_NAME, TABLE_NAME), RPAD(SCHEMA_NAME, SCHEMA_LEN) || CHAR(32) || RPAD(TABLE_NAME, TABLE_LEN) || CHAR(32) || RPAD(TABLE_TYPE, 6) || CHAR(32) || LPAD(PARTITIONS, 6) ||
    LPAD(RECORDS, 12) || LPAD(IFNULL(MEM_TOTAL_GB, 0.00), 13) || LPAD(IFNULL(MEM_DELTA_GB, 0.00), 13)  || LPAD(IFNULL(MEM_PAGE_GB, 0.00), 12) || LPAD(IFNULL(CURR_DISK_GB, 0.00), 13) ||
  ...
  ( SELECT
    ...
    ( SELECT
        ...
        TO_DECIMAL(SUM(T.MEMORY_SIZE_IN_PAGE_LOADABLE_MAIN / 1024 / 1024 / 1024), 10, 2) MEM_PAGE_GB,
        0 TABLE_SCANS_PER_S
      FROM
        ACCESSED_TABLES AC,
        M_CS_TABLES T
  ...
  SELECT TOP 1 4980, RPAD('SCHEMA_NAME', SCHEMA_LEN) || CHAR(32) || RPAD('TABLE_NAME', TABLE_LEN) || CHAR(32) || RPAD('INDEX_NAME', INDEX_LEN) || CHAR(32) ||
    RPAD('TY', 2) || CHAR(32) || LPAD('MEM_TOT_MB', 10) || CHAR(32) || LPAD('MEM_CONC_MB', 11) || CHAR(32) || LPAD('MEM_PERS_MB', 11) || CHAR(32) || LPAD('MEM_PAGE_MB', 11) || CHAR(32) ||
    LPAD('INDIV_COSTS', 11) || CHAR(32) || RPAD('INDIV_COLUMN', INDIV_COL_LEN) FROM ACCESSED_INDEXES UNION ALL
  SELECT TOP 1 4990, RPAD('=', SCHEMA_LEN, '=') || CHAR(32) || RPAD('=', TABLE_LEN, '=') || CHAR(32) || RPAD('=', INDEX_LEN, '=') || CHAR(32) ||
    RPAD('=', 2, '=') || CHAR(32) || RPAD('=', 10, '=') || CHAR(32) || RPAD('=', 11, '=') || CHAR(32) || RPAD('=', 11, '=') || CHAR(32) || RPAD('=', 11, '=') || CHAR(32) || 
    RPAD('=', 11, '=') || CHAR(32) || RPAD('=', INDIV_COL_LEN, '=') FROM ACCESSED_INDEXES UNION ALL
  SELECT 5000 + ROW_NUMBER() OVER (ORDER BY SCHEMA_NAME, TABLE_NAME, INDEX_NAME_NO_PART, PART_ID) / 100, RPAD(SCHEMA_NAME, SCHEMA_LEN) || CHAR(32) || RPAD(TABLE_NAME, TABLE_LEN) || CHAR(32) || RPAD(INDEX_NAME, INDEX_LEN) || CHAR(32) ||
    RPAD(TY, 2) || CHAR(32) || LPAD(MEM_TOT_MB, 10) || CHAR(32) || LPAD(MEM_CONC_MB, 11) || CHAR(32) || LPAD(MEM_PERS_MB, 11) || CHAR(32) || LPAD(MEM_PAGE_MB, 11) || CHAR(32) ||
    MAP(INDIV_COSTS, -1, LPAD('', 11), LPAD(INDIV_COSTS, 11)) || CHAR(32) || RPAD(INDIV_COLUMN, INDIV_COL_LEN)
  FROM
  ...
  SELECT TOP 1 6080, RPAD('SCHEMA_NAME', SCHEMA_LEN) || CHAR(32) || RPAD('TABLE_NAME', TABLE_LEN) || CHAR(32) || RPAD('COLUMN_NAME', COLUMN_LEN) || CHAR(32) || LPAD('NUM_DISTINCT', 12) ||
    ...
    RPAD('LOAD_UNIT', 9) || CHAR(32) || LPAD('SCANNED_RECS_PER_S', 18) || CHAR(32) || LPAD('INDEX_LOOKUPS_PER_S', 19) FROM ACCESSED_COLUMNS UNION ALL
  ...
  SELECT 6100 + ROW_NUMBER() OVER (ORDER BY SCHEMA_NAME, TABLE_NAME, COLUMN_NAME) / 100,
    ...
    RPAD(LOAD_UNIT, 9) || CHAR(32) || LPAD(MAP(SCANNED_RECS_PER_S, -1, 'n/a', TO_VARCHAR(SCANNED_RECS_PER_S)), 18) || CHAR(32) || LPAD(MAP(INDEX_LOOKUPS_PER_S, -1, 'n/a', TO_VARCHAR(INDEX_LOOKUPS_PER_S)), 19)
  FROM
    ACCESSED_COLUMNS
  UNION ALL
  ...
  SELECT TOP 1 6580, RPAD('SCHEMA_NAME', SCHEMA_LEN) || CHAR(32) || RPAD('TABLE_NAME', TABLE_LEN) || CHAR(32) || LPAD('MEM_SIZE_GB', 11) || CHAR(32) || 
    LPAD('RECORDS', 12) || CHAR(32) || RPAD('LOAD_UNIT', 9) || CHAR(32) || LPAD('READ_COUNT', 10) || CHAR(32) || LPAD('WRITE_COUNT', 11) || CHAR(32) || LPAD('MERGE_COUNT', 11) || CHAR(32) ||
    RPAD('LEVEL_1_PARTITIONING', L1_LEN) || CHAR(32) || RPAD('LEVEL_2_PARTITIONING', L2_LEN) FROM ACCESSED_PARTITIONS UNION ALL
  SELECT TOP 1 6590, RPAD('=', SCHEMA_LEN, '=') || CHAR(32) || RPAD('=', TABLE_LEN, '=') || CHAR(32) || LPAD('=', 11, '=') || CHAR(32) || 
    LPAD('=', 12, '=') || CHAR(32) || RPAD('=', 9, '=') || CHAR(32) || LPAD('=', 10, '=') || CHAR(32) || LPAD('=', 11, '=') || CHAR(32) || LPAD('=', 11, '=') || CHAR(32) ||
    RPAD('=', L1_LEN, '=') || CHAR(32) || RPAD('=', L2_LEN, '=') FROM ACCESSED_PARTITIONS UNION ALL
  SELECT 6600 + ROW_NUMBER() OVER (ORDER BY TP.SCHEMA_NAME, TP.TABLE_NAME, TP.PART_ID) / 100, 
    ...
    LPAD(TP.RECORDS, 12) || CHAR(32) || RPAD(TP.LOAD_UNIT, 9) || CHAR(32) || LPAD(TP.READ_COUNT, 10) || CHAR(32) || LPAD(TP.WRITE_COUNT, 11) || CHAR(32) || LPAD(TP.MERGE_COUNT, 11) || CHAR(32) ||
    RPAD(TP.LEVEL_1_PARTITIONING, L1_LEN) || CHAR(32) || RPAD(TP.LEVEL_2_PARTITIONING, L2_LEN)
