...
- Special table types:
  ...
  - BW shadow tables (ZBICZ*, ZBI0Z*, SAP Note 2155151)
  ...
  - SLT logging tables (/1CADMC/*, /1DH/ML*, /1DH/SL*)
...
- 2024/05/25:  2.2 (SLT logging tables added)
...
( SELECT                  /* Modification section */
    ...
    '%' SCHEMA_NAME,
    '%' TABLE_NAME,
    'SLT' APP_TABLE_TYPE,    /* ABAP_Z, BCP, BW_AGG, BW_SHADOW, BW_TEMP, ESH, OMR, POOL_CLUSTER, QCM, SLT, SUM_SWITCH, TABREP, TEXT_ANALYSIS, TEXT_MINING, % */
    -1 MIN_RECORD_COUNT,
    -1 MIN_SIZE_GB,
...
WHERE
  ...
  ( BI.APP_TABLE_TYPE IN ('%', 'ABAP_Z') AND T.SCHEMA_NAME LIKE 'SAP%' AND T.TABLE_NAME NOT LIKE 'ZARIX%' AND
    ...
    BI.APP_TABLE_TYPE IN ('%', 'BW_SHADOW') AND ( SUBSTR(T.TABLE_NAME, 1, 5) = 'ZBICZ' OR SUBSTR(T.TABLE_NAME, 1, 5) = 'ZBI0Z' ) OR
    ...
    BI.APP_TABLE_TYPE IN ('%', 'SLT') AND ( T.TABLE_NAME LIKE '/1CADMC/%' OR T.TABLE_NAME LIKE '/1DH/ML%' OR T.TABLE_NAME LIKE '/1DH/SL%' ) OR
