...
- HANA_Tables_TopGrowingTables_Size_History_2.00.043+
...
- 2023/02/02:  1.8 (adjusted version to 2.00.043+ instead of 2.00.030+ due to HOST_COLUMN_TABLES_PART_SIZE.PERSISTENT_MEMORY_SIZE_IN_TOTAL)
...
( SELECT
  ...
  ( SELECT
    ...
    ( SELECT                         /* Modification section */
        ...
        'TABLE' AGGREGATE_BY,         /* SCHEMA, TABLE, HOST, PORT */
  ...
  ( SELECT
    ...
    ( SELECT
      ...
      ( SELECT
          ...
          TE.SCHEMA_NAME,
          TE.TABLE_NAME,
          SUM(TE.DISK_SIZE) SIZE_END
        FROM
          BASIS_INFO BI,
          _SYS_STATISTICS.GLOBAL_TABLE_PERSISTENCE_STATISTICS TE
        WHERE
          ...
          UPPER(TE.TABLE_NAME) LIKE UPPER(BI.TABLE_NAME)
        GROUP BY
          TE.HOST,
          TE.PORT,
          TE.SCHEMA_NAME,
      ...
      ( SELECT
          ...
          TB.SCHEMA_NAME,
          TB.TABLE_NAME,
          SUM(TB.DISK_SIZE) SIZE_BEGIN
        FROM
          BASIS_INFO BI,
          _SYS_STATISTICS.GLOBAL_TABLE_PERSISTENCE_STATISTICS TB
        WHERE
          TB.SERVER_TIMESTAMP = BI.DISK_BEGIN_TIME AND
          TB.HOST LIKE BI.HOST AND
          TO_VARCHAR(TB.PORT) LIKE BI.PORT AND
          UPPER(TB.SCHEMA_NAME) LIKE UPPER(BI.SCHEMA_NAME) AND
          UPPER(TB.TABLE_NAME) LIKE UPPER(BI.TABLE_NAME)
        GROUP BY
          TB.HOST,
          TB.PORT,
          TB.SCHEMA_NAME,
