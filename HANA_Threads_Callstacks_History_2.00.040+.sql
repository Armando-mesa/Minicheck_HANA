...
- HANA_Threads_Callstacks_History_2.00.040+
...
- 2023/03/16:  1.6 (ONLY_CHECK_TABLE_CONSISTENCY_CALLSTACKS included)
- 2023/06/25:  1.7 (ONLY_DELTA_STORAGE_CALLSTACKS included)
...
( SELECT
  ...
  ( SELECT                     /* Modification section */
      'C-D42' BEGIN_TIME,                  /* YYYY/MM/DD HH24:MI:SS timestamp, C, C-S<seconds>, C-M<minutes>, C-H<hours>, C-D<days>, C-W<weeks>, E-S<seconds>, E-M<minutes>, E-H<hours>, E-D<days>, E-W<weeks>, MIN */
      ...
      80 MAX_CALL_LEN,
      'X' SHORT_CALLS,
      'CALLSTACK' AGGREGATE_BY,     /* TIME, HOST, PORT, SERVICE, THREAD_ID, CALLSTACK, HASH or comma separated combinations, NONE for no aggregation */
...
( SELECT '%AttributeEngine::fnJeReadIndexIndividual%' METHOD FROM DUMMY UNION ALL
  ...
  SELECT '%TRexUtils::Mset%' FROM DUMMY UNION ALL
  ...
  WHERE
    ...
    ( BI.ACTIVITY_TYPE = '%' OR
      ...
      ( SUBSTR(TC.CALLSTACK_CONCAT, 1, 7) IN ( 'do_fute', 'io_queu', '__nanos', '__pthre', 'syscall' ) OR 
        SUBSTR(TC.CALLSTACK_CONCAT, 1, 17) = '__clock_nanosleep' OR
        SUBSTR(TC.CALLSTACK_CONCAT, 1, 21) = '.annobin_syslog.c_end'
      ) OR
