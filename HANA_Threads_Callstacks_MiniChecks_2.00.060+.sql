...
- HANA_Threads_Callstacks_MiniChecks_2.00.060+
...
- 2022/05/27:  1.3 (dedicated 2.00.060+ version including SITE_ID for data source HISTORY)
...
SELECT
  CASE WHEN ROW_NUMBER() OVER (PARTITION BY CHECK_ID ORDER BY SITE_ID, HOST, PORT) = 1 THEN CASE WHEN CHECK_ID >= 100 THEN 'C' || LPAD(CHECK_ID, 4, '0') ELSE '' END ELSE '' END CHID,
  CASE WHEN ROW_NUMBER() OVER (PARTITION BY CHECK_ID ORDER BY SITE_ID, HOST, PORT) = 1 THEN AREA ELSE '' END AREA,
  CASE WHEN ROW_NUMBER() OVER (PARTITION BY CHECK_ID ORDER BY SITE_ID, HOST, PORT) = 1 THEN DESCRIPTION ELSE '' END DESCRIPTION,
  CASE WHEN ROW_NUMBER() OVER (PARTITION BY CHECK_ID, SITE_ID ORDER BY HOST, PORT ) = 1 THEN IFNULL(LPAD(SITE_ID, 2), '') ELSE '' END ST,
  CASE WHEN ROW_NUMBER() OVER (PARTITION BY CHECK_ID, SITE_ID, HOST ORDER BY PORT) = 1 THEN HOST ELSE '' END HOST,
