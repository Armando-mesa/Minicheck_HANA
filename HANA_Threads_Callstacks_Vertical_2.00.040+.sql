...
( SELECT                     /* Modification section */
    ...
    ' ' SHORT_CALLS,
    'CALLSTACK' AGGREGATE_BY     /* HOST, PORT, SERVICE, CONN_ID, THREAD_ID, HASH, THREAD_STATE, THREAD_TYPE, THREAD_METHOD, THREAD_DETAIL, LOCK_NAME, CALLER, CALLING, CALLSTACK or comma separated combinations, NONE for no aggregation */
  FROM
    DUMMY
),
THREADS AS
    ...
    ( SELECT
      ...
      WHERE
        ...
        ( BI.ACTIVITY_TYPE = '%' OR
          ...
          BI.ACTIVITY_TYPE IN ( 'NETWORK', 'WAIT' ) AND SUBSTR(TCC.CALLSTACK_CONCAT, 1, 7) IN ( '__GI___', '__GI_ep', '__GI_po', '__epoll', '__libc_', '__poll_', '__read_', '__write' ) OR
          BI.ACTIVITY_TYPE = 'CPU'                  AND SUBSTR(TCC.CALLSTACK_CONCAT, 1, 7) NOT IN ( 'do_fute', '__GI___', '__GI_ep', '__GI_po', 'io_queu', '__epoll', '__libc_', '__nanos', '__poll_', '__pthre', '__read_', 'syscall', '__write' )
        ) AND
